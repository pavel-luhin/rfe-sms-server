(function () {
    ...
    angular
        .module('sms-server')
        .controller('addPersonsCtrl', addPersonsCtrl);

    addPersonsCtrl.$inject = ['addPersonsService', '$scope', '$location', 'confirmService', '$routeParams'];
    function addPersonsCtrl(addPersonsService, $scope, $location, confirmService, $routeParams) {
        var sortConstants = {
            notSorted: {
        ...
        $scope.addPersons = function (recipients) {
            addPersonsService.savePersons(recipients)
                .then(function (data) {
        ...
        $scope.removePerson = function (id) {
            confirmService.showModal({}, confirmDeleteModalOptions).then(function (result) {
                addPersonsService.removePerson(id).then(function () {
                    $scope.getAllPersons();
                });
            });
        };
        ...
        $scope.getAllPersons = function () {
            addPersonsService.getPersons(skip, currentPageSize, sortField, sortDirection).then(function (response) {
